...
def prepare_dataset(df: pd.DataFrame, horizon: int = 5, threshold: float = 0.005) -> Tuple[pd.DataFrame, pd.Series]:
    df = df.copy()
    ...
    return df[available], df["label"]


def train_random_forest(
    df: pd.DataFrame, symbol: str, horizon: int = 5,
    n_estimators: int = 200, max_depth: int | None = 12, version: str = "v1",
) -> Dict[str, Any]:
    ...
    return {
        "model_type": "rf", "symbol": symbol, "version": version,
        "file_path": str(model_path), "accuracy": float(acc),
        "precision": float(prec), "recall": float(rec), "f1": float(f1),
        "cv_mean_accuracy": float(cv_scores.mean()),
        "hyperparameters": {"n_estimators": n_estimators, "max_depth": max_depth, "horizon": horizon},
